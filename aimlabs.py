...
for sys_font in sys_fonts:
    fonts[sys_font] = pygame.font.SysFont(sys_font,font_size) 
current_font = def_font if def_font in fonts else back_up_font if back_up_font in fonts else sys_fonts[0]
...
font_file_rect = pygame.rect.Rect((font_spot.right+sett_v_gap,font_spot.top),(changing_display_width,button_dimens[1]))
font_file_rect.centery = font_spot.centery 
font_files_color = "Grey"

font_change_trigger = False
    #endregion
...
while running:
    ...
    button_triggers = [typing_trigger,sett_trigger,start_trigger] + list(suggestions_trigger_dict.values()) +[back_change_trigger,font_change_trigger]+[game_mode_trigger,high_score_trigger]
    ...
    font_switch_button = def_button(font_file_rect.topright,["CHANGE","CHANGE?"])

    back_switch_button = def_button(back_imgfile_rect.topright,["CHANGE","CHANGE?"])
    ...
    #region Settings           
        ...
        if back_img_paths:
            ...
            back_change_trigger = gamefuncs.Button.draw(back_switch_button)
        #endregion
        ...
        font_change_trigger = gamefuncs.Button.draw(font_switch_button)
        #endregion
    #endregion
